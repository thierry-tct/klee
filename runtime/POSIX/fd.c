...
static exe_disk_file_t *__get_sym_file_internal(const char *pathname, int flags,
                                                bool cp_treat_symbolic);
...
static exe_disk_file_t *__get_sym_file_internal(const char *pathname, int flags,
  ...
  if (!pathname[1]) { // standard one-letter symbolic file
    ...
    if (klee_is_symbolic(pathname[0])) {
      klee_warning("ZEST: opening symbolic file\n");
      file_check = true;
    } else {
    ...
    if (file_check) {
      ...
      for (i = 0; i < __exe_fs.n_cp_files; ++i) {
        if (__exe_fs.cp_files[i].path &&
            0 == strcmp(__exe_fs.cp_files[i].path, pathname)) {
  ...
  if (df) {    
    ...
    if (strncmp(pathname, "/proc/self/fd/", 14) == 0) {
      ...
      if (f) {
        ...
        snprintf(buffer, sizeof(buffer) - 1, "/proc/self/fd/%d%s", f->fd,
                 (pathtail ? pathtail : ""));
...
void *mmap64(void *addr, size_t length, int prot, int flags, int fd,
  ...
  if (f->dfile) {
    ...
    long r = syscall(__NR_mmap, addr, length, prot, flags, f->fd, off);
    if (r == -1)
      errno = klee_get_errno();
    return (void *)r;
  }
...
int futimens(int fd, const struct timespec times[2]) {
  ...
  int r = syscall(__NR_utimensat, (long)f->fd, 0L, times, 0L);
  if (r == -1)
    errno = klee_get_errno();
  return r;
}
...
int link(const char *oldpath, const char *newpath) {
  ...
  int r = syscall(__NR_link, __concretize_string(oldpath),
                  __concretize_string(newpath));
  if (r == -1)
    errno = klee_get_errno();
  return r;
}
...
int symlink(const char *oldpath, const char *newpath) {
  ...
  int r = syscall(__NR_symlink, __concretize_string(oldpath),
                  __concretize_string(newpath));
  if (r == -1)
    errno = klee_get_errno();
  return r;
...
#if USE_TRANSPARENT_FDS
    klee_warning_once(
        "Cannot guarantee dup semantics with transparent file descriptors\n");
#endif
    int fd;
    for (fd = 0; fd < MAX_FDS; ++fd)
      if (!(__exe_env.fds[fd].flags & eOpen))
...
int select(int nfds, fd_set *read, fd_set *write,
  ...
  // sprintf(msg, "model select for %d fds. timeout %d s %d us", nfds, sec,
  // usec);
  // klee_warning(msg);
      ...
      if (!f) {
        ...
        if (FD_ISSET(i, &in_read)) {
          ...
          if (!(f->flags & eSocket))
            ...
            flags |= (__exe_fs.n_sym_dgrams_used < __exe_fs.n_sym_dgrams)
                         ? 01
                         : 0; /* more dgrams available */
          else if (f->flags & eListening)
            flags |= (__exe_fs.n_sym_streams_used < __exe_fs.n_sym_streams)
                         ? 01
                         : 0; /* more streams available */
          else
            flags |= 01;
          if (flags & 01)
            FD_SET(i, read);
...
int pipe(int filedes[2]) {
  ...
  f[0]->fd = os_filedes[0];
  f[0]->flags = eOpen | eReadable;
  f[1]->fd = os_filedes[1];
  f[1]->flags = eOpen | eWriteable;
  fprintf(stderr, "KLEE: MODEL: pipe: returning {%d,%d}->{%d,%d}\n", filedes[0],
          filedes[1], os_filedes[0], os_filedes[1]);
  return 0;
}
