...
void __fill_blocks(exe_disk_file_t *dfile, const fill_info_t *fill_info,
                   unsigned n_fill_info, int do_memset) {
  unsigned i, j;
  int s;
  char *file;
  for (i = 0; i < n_fill_info; i++) {
    ...
    case fill_set:
      /* effectively memset(dfile->contents + p->offset, p->arg.value,
       * p->length) */
      if (do_memset) {
      ...
      break;
    case fill_copy:
      /* effectively memcpy(dfile->contents + p->offset, p->arg.string,
       * p->length) */
      if (do_memset) {
...
static void __create_new_dfile(exe_disk_file_t *dfile, unsigned size, 
                               char *contents, const char *name, 
                               const fill_info_t *fill_info,
                               unsigned n_fill_info, struct stat64 *defaults,
                               int is_foreign) {
...
void klee_init_fds(unsigned n_files, unsigned file_length,
                   ...
                   unsigned max_failures, int one_line_streams,
                   const fill_info_t stream_fill_info[],
                   unsigned n_stream_fill_info,
                   const fill_info_t dgram_fill_info[],
                   unsigned n_dgram_fill_info) {
  unsigned k;
  char name[7] = "?-data";
  char sname[] = "STREAM?";
  char dname[] = "DGRAM?";
  ...
  for (k=0; k < n_files; k++) {
    name[0] = 'A' + k;
    __create_new_dfile(&__exe_fs.sym_files[k], file_length, NULL, name, NULL, 0,
                       &s, 0);
  }
  ...
  if (stdin_length) {
    ...
    __create_new_dfile(__exe_fs.sym_stdin, stdin_length, NULL, "stdin", NULL, 0,
                       &s, 0);
  ...
  for (k = 0; k < n_streams; k++) {
    ...
    __create_new_dfile(&__exe_fs.sym_streams[k], stream_len, NULL, sname,
                       __stream_fill_info, __n_stream_fill_info, &s, 1);
  }
  ...
  for (k = 0; k < n_dgrams; k++) {
    ...
    __create_new_dfile(&__exe_fs.sym_dgrams[k], dgram_len, NULL, dname,
                       __dgram_fill_info, __n_dgram_fill_info, &s, 1);
  }
  __exe_fs.n_sym_dgrams_used = 0;

  /* patch __exe_env file offsets. otherwise we get unexpected behavior with
   * shared files (e.g., klee ls.bc >> out) */
  unsigned i;
  off_t offset;
  for (i = 0; i < 3; ++i) {
...
exe_disk_file_t *klee_init_cp_file(const char *path, int flags) {
  ...
  for (k = 0; k < __exe_fs.n_cp_files; k++) {
    if (!__exe_fs.cp_files[k].stat) {
      __create_new_dfile(&__exe_fs.cp_files[k], fsize, buf, path, NULL, 0, &def,
                         0);
