...
usage: (klee_init_env) [options] [program arguments]\n\
  ...
  while (k < argc) {
	...
	__emit_error(msg);
    ...
    } else if (__streq(argv[k], "--one-line-streams") ||
               __streq(argv[k], "-one-line-streams")) {
      one_line_streams_flag = 1;
      k++;
    } else if (__streq(argv[k], "--fill-streams") ||
               __streq(argv[k], "-fill-streams")) {
      const char *msg = "--fill-streams expects arguments <offset> \"set\" "
                        "<length> <value> or <offset> \"copy\" <string> or "
                        "<offset> \"file\" <string>";
      const char *msg2 = "--fill-streams: too many blocks";

      if (n_stream_fill_info >=
          sizeof(stream_fill_info) / sizeof(*stream_fill_info))
        __emit_error(msg2);
      if (k + 2 >= argc)
        __emit_error(msg);
      k++;
      stream_fill_info[n_stream_fill_info].offset =
          __str_to_int(argv[k++], msg);
      if (__streq(argv[k], "set")) {
        ...
        stream_fill_info[n_stream_fill_info].length =
            __str_to_int(argv[k++], msg);
        stream_fill_info[n_stream_fill_info].arg.value =
            __str_to_int(argv[k++], msg);
        n_stream_fill_info++;
        ...
        stream_fill_info[n_stream_fill_info].length =
            __convert_escape_sequences(
                stream_fill_info[n_stream_fill_info].arg.string);
        n_stream_fill_info++;
      } else if (__streq(argv[k], "file")) {
        k++;
        if (k >= argc)
          __emit_error(msg);
        stream_fill_info[n_stream_fill_info].fill_method = fill_file;
        stream_fill_info[n_stream_fill_info].arg.string = argv[k++];
        n_stream_fill_info++;
      } else
        __emit_error(msg);

    } else if (__streq(argv[k], "--fill-datagrams") ||
               __streq(argv[k], "-fill-datagrams")) {
      const char *msg = "--fill-datagrams expects arguments <offset> \"set\" "
                        "<length> <value> or <offset> \"copy\" <string>";
      const char *msg2 = "--fill-datagrams: too many blocks";

      if (n_dgram_fill_info >=
          sizeof(dgram_fill_info) / sizeof(*dgram_fill_info))
        __emit_error(msg2);
      if (k + 2 >= argc)
        __emit_error(msg);
      k++;
      ...
      if (__streq(argv[k], "set")) {
        ...
        dgram_fill_info[n_dgram_fill_info].length =
            __str_to_int(argv[k++], msg);
        dgram_fill_info[n_dgram_fill_info].arg.value =
            __str_to_int(argv[k++], msg);
        n_dgram_fill_info++;
      } else if (__streq(argv[k], "copy")) {
        k++;
        ...
        dgram_fill_info[n_dgram_fill_info].length = __convert_escape_sequences(
            dgram_fill_info[n_dgram_fill_info].arg.string);
        n_dgram_fill_info++;
      } else if (__streq(argv[k], "file")) {
        k++;
        if (k >= argc)
          __emit_error(msg);
        dgram_fill_info[n_dgram_fill_info].fill_method = fill_file;
        dgram_fill_info[n_dgram_fill_info].arg.string = argv[k++];
        n_dgram_fill_info++;
      } else
        __emit_error(msg);
  ...
  *argvPtr = final_argv;
  ...
  klee_init_fds(sym_files, sym_file_len, sym_stdin_len, sym_stdout_flag,
                save_all_writes_flag, sym_streams, sym_stream_len, sym_dgrams, sym_dgram_len, fd_fail,
                one_line_streams_flag, stream_fill_info, n_stream_fill_info,
